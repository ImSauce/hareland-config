...
"" "" \
"[Taskwarrior]" "" \
"sudo pacman -S task" "install taskwarrior" \
"task add 'Finish assignment' due:tomorrow" "add task with due date" \
"task" "view task list" \
"task 1 done" "mark task as done" \
"task 1 delete" "delete task" \
"task add 'Read docs' +school due:2025-06-15" "add task with tag and due date" \
"task +school" "filter tasks by tag" \
"task list" "show list report" \
"task long" "show detailed report" \
"task next" "show next recommended tasks" \
"task 2 modify 'Read Zoxide docs'" "modify task description" \
"task 2 due:tomorrow" "modify task due date" \
"task gc" "clean up completed/deleted tasks" \
"task status:pending delete" "delete all active/pending tasks" \
"rm -rf ~/.task" "wipe all taskwarrior data (pending, completed, undo)" \
"task" "reinitialize Taskwarrior config if deleted" \
"" "" \
"" "" \
"[Asciinema]"  "" \
"asciinema rec {dir/filename.cast}" "start recording terminal session" \
...
"hyprctl clients | grep -A 10 {searchword}" "find the class name for the app so you can change the icon in ModulesWorkspace.sh or just use [hyprctl clients] to search all open windows" \
"fd -i/-e {search}" "search for a file or file type with ignored case" \
"lshw -C {search}" "info about computer parts like memory, disk, network" \
"duf" "show drive usage with better formatting" \
"history | fzf" "search in the commands history" \
"ranger" "terminal file manager" \
"stat {file}" "show info about that file" \
"z/zi" "shortcut better than cd" \
