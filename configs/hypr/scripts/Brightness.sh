...
change_backlight() {
	...
	# Calculate new brightness
	if (( current_brightness == 5 )); then
		if [[ "$1" == "+${step}%" ]]; then
			new_brightness=$((current_brightness + step))   # +5
		elif [[ "$1" == "${step}%-" ]]; then
			new_brightness=$((current_brightness - 1))      # -1
		fi
	elif (( current_brightness < 5 )); then
		if [[ "$1" == "+${step}%" ]]; then
			new_brightness=$((current_brightness + 1))      # +1
		elif [[ "$1" == "${step}%-" ]]; then
	...
	# Ensure brightness doesn't go below 1 or above 100
